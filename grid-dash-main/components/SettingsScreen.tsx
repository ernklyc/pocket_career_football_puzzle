    ...
    <div className="flex flex-col h-[100svh] p-2 sm:p-4 font-sans fade-in bg-slate-200">
      ...
      <div className="relative w-full max-w-md mx-auto flex flex-col h-full">
        <header className="sticky top-0 sm:top-2 z-20 w-full flex-shrink-0 flex items-center justify-center p-3 mb-4 bg-white/60 backdrop-blur-xl border border-white/30 rounded-2xl shadow-lg">
          ...
          <h1 className="text-2xl font-black text-slate-700">Ayarlar</h1>
        </header>
...
const SettingsSlider: React.FC<SettingsSliderProps> = ({ label, icon, value, onChange }) => {
    return (
        <div className="bg-white/70 backdrop-blur-xl border border-white/20 shadow-md p-4 rounded-2xl">
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-4 text-slate-700">
                    {icon}
                    <span className="font-bold text-lg">{label}</span>
                </div>
                <span className="font-mono text-sm text-slate-500 bg-slate-200 px-2 py-1 rounded-md">
            ...
            />
             <style>{`
                .range-slider {
                    background-image: linear-gradient(to right, #38bdf8 var(--value), #e2e8f0 var(--value));
                }
...
const SettingsToggle: React.FC<SettingsToggleProps> = ({ label, icon, isEnabled, onToggle, isDisabled, disabledTooltip }) => {
    return (
        <div className={`flex items-center justify-between p-4 rounded-2xl transition-colors backdrop-blur-xl border border-white/20 shadow-md ${isDisabled ? 'bg-slate-200/60' : 'bg-white/70'}`}>
            <div className={`flex items-center gap-4 ${isDisabled ? 'text-slate-400' : 'text-slate-700'}`}>
                {icon}
                <span className="font-bold text-lg">{label}</span>
            </div>
            <button
...
const SettingsButton: React.FC<SettingsButtonProps> = ({ label, icon, onClick, isDanger }) => {
    const textColor = isDanger ? 'text-rose-600' : 'text-slate-700';
    const bgColor = isDanger ? 'bg-rose-100/70' : 'bg-white/70';
