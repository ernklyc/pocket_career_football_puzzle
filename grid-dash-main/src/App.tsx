...
// FIX: Correctly import DailyRewardModal as a named export. The file has no default export.
...
const AppContent: React.FC = () => {
  ...
  // Save progress whenever it changes and sync to Firebase
  useEffect(() => {
    ...
    syncTimeoutRef.current = window.setTimeout(async () => {
        if (progress.userId) {
            try {
                await setDoc(doc(db, "players", progress.userId), {
                    name: progress.playerName,
                    gems: progress.gems,
                    weeklyGems: progress.weeklyGems,
    ...
    }, 3000); // Sync every 3 seconds of inactivity to save writes
          ...
          if (isLoading || !currentLevel) {
            return (
              <div className="flex flex-col items-center justify-center min-h-screen p-4 text-center">
                ...
                <h2 className="text-2xl font-bold text-text-primary">Seviye Oluşturuluyor...</h2>
              </div>
            );
          }
          return (
