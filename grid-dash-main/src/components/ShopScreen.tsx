...
const AssetDisplay: React.FC<{ icon: React.ReactNode, label: string, value: number }> = ({ icon, label, value }) => (
  <div>
    <div className="w-10 h-10 bg-slate-200/70 rounded-full flex items-center justify-center mx-auto mb-1.5 text-text-secondary">{icon}</div>
    ...
    <div className="flex flex-col h-[100svh] p-2 sm:p-4 font-sans fade-in bg-background">
      ...
      <div className="relative w-full max-w-md mx-auto flex flex-col h-full">
        <header className="sticky top-0 sm:top-2 z-20 w-full flex-shrink-0 flex items-center justify-center p-3 mb-4 bg-surface/60 backdrop-blur-xl border border-white/30 rounded-2xl shadow-lg">
          ...
          <h1 className="text-2xl font-black text-text-primary">Dükkan</h1>
        </header>

        <main className="w-full flex-1 overflow-y-auto scrollbar-hide px-2 pb-4 space-y-6">
          <div className={`bg-surface/70 backdrop-blur-xl border border-white/20 shadow-md p-4 rounded-2xl transition-transform duration-300 ${shakeGems ? 'animate-shake' : ''}`} onAnimationEnd={() => setShakeGems(false)}>
            <h2 className="text-sm font-bold text-text-secondary uppercase tracking-wider mb-3 px-1">Varlıklarım</h2>
          ...
          <div>
            <h2 className="text-sm font-bold text-text-secondary uppercase tracking-wider mb-2 px-2">Elmas Satın Al</h2>
            <div className="grid grid-cols-2 gap-3">
              {GEM_PACKAGES.map(pack => (
                <div key={pack.id} className="relative bg-surface/70 backdrop-blur-xl border border-white/20 shadow-md p-4 rounded-2xl flex flex-col items-center text-center">
                  {pack.bestValue && <div className="absolute top-0 -mt-2.5 bg-danger text-white text-xs font-bold px-3 py-1 rounded-full uppercase shadow-lg">En İyi Değer</div>}
                  <div className={`w-16 h-16 ${pack.color} rounded-xl flex items-center justify-center mb-3`}>
                    <pack.icon size={32} />
                  </div>
                  <p className="text-2xl font-black text-text-primary">{pack.amount.toLocaleString()}</p>
                  <p className="text-sm text-text-secondary font-semibold mb-3">Elmas</p>
                  <button
          ...
          <div>
            <h2 className="text-sm font-bold text-text-secondary uppercase tracking-wider mb-2 px-2">Güçlendirmeler</h2>
            <div className="space-y-3">
              {Object.entries(SHOP_ITEMS).map(([id, item]) => {
                const Icon = item.icon;
                return (
                  <div key={id} className="bg-surface/70 backdrop-blur-xl border border-white/20 shadow-md p-4 rounded-2xl flex items-center gap-4">
                    ...
                    <button 
                      ...
                      className="flex flex-col items-center justify-center gap-1 bg-secondary text-white font-bold px-4 py-2 rounded-xl transition-all duration-200 transform active:scale-95 hover:bg-blue-600 shadow-md"
                    >
