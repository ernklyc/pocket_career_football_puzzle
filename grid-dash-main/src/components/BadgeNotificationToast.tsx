...
const BadgeNotification: React.FC<{ badge: Badge; onDismiss: (id: string) => void }> = ({ badge, onDismiss }) => {
    ...
    return (
        <div
            ...
            className="relative w-full bg-surface rounded-xl border border-slate-200 p-4 flex items-center gap-4 shadow-lg cursor-grab active:cursor-grabbing touch-none transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] transition-opacity"
        >
            <div className="p-3 rounded-full bg-sky-100 shrink-0">
                <Icon className="w-8 h-8 text-sky-600" />
            </div>
            <div className="flex-grow min-w-0">
                <p className="text-text-secondary text-sm font-semibold">Başarım Kazanıldı!</p>
                <h3 className="text-text-primary font-bold text-lg truncate">{badge.name}</h3>
            </div>
            <button
