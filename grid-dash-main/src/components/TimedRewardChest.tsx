...
const DailyRewardIndicator: React.FC<DailyRewardIndicatorProps> = ({ nextRewardTime, onClaim }) => {
  ...
  return (
    <div className="bg-surface/70 backdrop-blur-xl border border-white/20 shadow-md p-4 rounded-2xl flex items-center justify-between">
      <div className="flex items-center gap-4">
        <div className="text-text-secondary">
          <ClockIcon />
        </div>
        <div>
          <h3 className="font-bold text-text-primary">Günlük Ödül</h3>
          <p className="text-sm text-text-secondary">Sonraki ödül</p>
        </div>
      </div>
      <div className="font-mono text-xl text-secondary font-bold bg-sky-100 px-3 py-1 rounded-md">
        {formattedTime}
      </div>
    </div>
  );
