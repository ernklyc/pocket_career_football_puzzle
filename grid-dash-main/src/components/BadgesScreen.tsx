...
const BadgeCard: React.FC<{ badge: Badge; isEarned: boolean }> = ({ badge, isEarned }) => {
  ...
  if (isEarned) {
    ...
    return (
      <div className="bg-surface/80 backdrop-blur-xl border-2 border-sky-300 shadow-lg rounded-2xl p-4 flex flex-col items-center text-center transition-all h-48">
        ...
        <div className="flex flex-col flex-grow items-center justify-center">
            <h3 className="font-bold text-base text-text-primary leading-tight">{badge.name}</h3>
            <p className="text-xs text-text-secondary font-medium mt-1 px-1">{badge.description}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-surface/50 backdrop-blur-xl border border-white/20 shadow-sm rounded-2xl p-4 flex flex-col items-center justify-center text-center transition-all h-48 opacity-70">
        ...
        <div className="flex flex-col items-center justify-center">
            <h3 className="font-bold text-base text-text-secondary mb-1">Gizli Başarım</h3>
...
const BadgesScreen: React.FC<BadgesScreenProps> = ({ progress, onBack }) => {
  ...
  return (
    <div className="flex flex-col h-[100svh] font-display bg-background">
      ...
      <div className="relative w-full max-w-lg mx-auto flex flex-col h-full">
        <header className="sticky top-0 sm:top-2 z-20 w-full flex-shrink-0 flex items-center justify-center p-3 my-2 bg-surface/60 backdrop-blur-xl border border-white/30 rounded-2xl shadow-lg">
            ...
            <h1 className="text-2xl font-black text-text-primary">Başarımlar</h1>
        </header>
