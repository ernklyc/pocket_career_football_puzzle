...
const GameScreen: React.FC<GameScreenProps> = ({ level, onLevelComplete, onBack, hints, onUseHint, timeShifts, onUseTimeShift, solverPieces, onUseSolverPiece, gems, hasSeenTutorial, onTutorialComplete }) => {
  ...
  useEffect(() => {
    if (!isCompletingRef.current && totalGridCells > 0 && filledGridCells === totalGridCells && availableBlocks.length === 0) {
      ...
      if (dropContainerRef.current) {
        const rect = dropContainerRef.current.getBoundingClientRect();
        triggerParticles(rect.left + rect.width / 2, rect.top, 'confetti');
      }
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (isValid) {
      ...
      if (newlyCollectedGems.length > 0) {
        ...
        gemsCollectedThisLevel.current += newlyCollectedGems.length;
        setDisplayGems(prev => prev + newlyCollectedGems.length);
        addLogEntry(`+${newlyCollectedGems.length} Elmas`, 'gem', DiamondIcon);
      }
        ...
        <div className="flex justify-between items-center bg-black/10 backdrop-blur-2xl rounded-2xl p-2 sm:p-4 shadow-lg border border-white/20">
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div className="flex-shrink min-w-0">
              {level.name ? (
                <h1 className="text-xl sm:text-2xl font-bold text-white leading-tight truncate" title={level.name}>
                  {level.name}
                </h1>
              ) : (
                <div className="h-7 w-40 sm:w-56 bg-white/20 rounded-md animate-pulse"></div>
              )}
              <p className="text-sm text-white/70 font-semibold">Seviye {level.levelNumber}</p>
            </div>
          </div>
          <div ref={powerUpsRef} className="flex items-center gap-2 md:gap-4">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl font-semibold transition-all shadow-md text-white ${
                hints > 0
            ...
            >
              ...
              <span className="hidden sm:inline">İpucu</span>
               <span className="px-2 py-0.5 bg-black/20 rounded-full text-sm">{hints}</span>
            </button>
            ...
            >
                ...
                <span className="hidden sm:inline">Zaman</span>
                 <span className="px-2 py-0.5 bg-black/20 rounded-full text-sm">{timeShifts}</span>
            </button>
            <button
                ref={solverButtonRef}
            ...
            >
                ...
                <span className="hidden sm:inline">Çöz</span>
                 <span className="px-2 py-0.5 bg-black/20 rounded-full text-sm">{solverPieces}</span>
            </button>
          </div>
        </div>
      </header>
