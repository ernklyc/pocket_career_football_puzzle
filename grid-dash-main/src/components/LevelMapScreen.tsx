        ...
        >
            ...
            >
                <div className={`
                    ...
                    ${!isUnlocked ? 'bg-slate-300/70' : 'bg-surface/70 backdrop-blur-xl border border-white/20 shadow-md group-hover:shadow-lg group-active:scale-95'}
                    ${isCurrent ? 'animate-pulse-glow ring-2 ring-secondary' : ''}
                ...
                `}>
                    <div className={`
                        ...
                        ${!isCurrent && !isCompleted && isUnlocked ? 'bg-slate-100 text-text-secondary' : ''}
                    ...
                    <div className="flex-1 text-left min-w-0">
                        <h3 className={`font-bold text-lg truncate ${!isUnlocked ? 'text-slate-500' : isCompleted ? 'text-white' : 'text-text-primary'}`}>
                        ...
                        {isCompleted ? (
                            ...
                            <p className="text-sm text-secondary font-semibold">Sıradaki Macera!</p>
                        ) : isUnlocked ? (
                            <p className="text-sm text-text-secondary">Oynamaya Hazır</p>
                        ) : (
                            <p className="text-sm text-slate-500">Kilitli</p>
                        )}
                    </div>
                    {isUnlocked && !isCompleted && <div className="text-secondary opacity-30 group-hover:opacity-100 transition-opacity"><PlayIcon size={28}/></div>}
    ...
    <div className="flex flex-col h-[100svh] p-2 sm:p-4 font-sans fade-in bg-background">
      ...
      <div className="relative w-full flex flex-col h-full">
        <main className="flex-1 overflow-y-auto scrollbar-hide">
            <header className="sticky top-0 sm:top-2 z-20 flex items-center justify-center p-3 mb-4 shrink-0 bg-surface/60 backdrop-blur-xl border border-white/30 rounded-2xl shadow-lg w-full max-w-md md:max-w-2xl mx-auto">
              ...
              <h1 className="text-2xl font-black text-text-primary">Seviyeler</h1>
            </header>
            ...
            <div className="flex flex-col items-center gap-6 relative px-2 py-4 w-full max-w-md md:max-w-2xl mx-auto">
                ...
                {/* Current Level */}
                <div className="w-full">
                    <h2 className="text-sm font-bold text-secondary uppercase tracking-wider mb-2 px-2">Sıradaki Seviye</h2>
                    <LevelCard
                ...
                {completedLevels.length > 0 && (
                    <div className="w-full">
                        <h2 className="text-sm font-bold text-text-secondary uppercase tracking-wider mb-2 px-2">Tamamlananlar</h2>
